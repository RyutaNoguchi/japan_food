...
(function ($) {
  ...
  $.fn.jmapBlackOrWhite = function (_hexcolor) {
    ...
    if (hexcolor.length <= 4) {
      ...
      if (hexArray.length >= 4) hexArray.pop();
      hexcolor = "#";
  ...
  $.fn.jmap = function (options) {
    ...
    var params = $.extend({
      ...
      onHover: function onHover(e, data) {},
      onMouseout: function onMouseout(e, data) {},
      onLoad: function onLoad(e, data) {},
      onSelectHeatMapLabel: function onSelectHeatMapLabel(e, data) {},
      onHoverHeatMapLabel: function onHoverHeatMapLabel(e, data) {},
      onLoadHeatMapLabel: function onLoadHeatMapLabel(e, data) {}
    }, options);
    ...
    if (params.id == null) params.id = uniqClass;
    if (params.heatmapColors.length <= 0) params.heatmapColors = conf.heatmap[params.heatmapType];
    if (params.heatmapConditions.length > 0) params.heatmapConditions = params.heatmapConditions.reverse();
    if (params.showHeatmap && params.showHeatlabel) params.showInfobox = true;
    if (!params.selectable) params.cursor = 'default'; // Jmap container
    ...
    var containerDiv = $('<div>').attr('jmap-id', params.id).addClass(params.containerClass); // Jmap content
    ...
    if (params.viewType == 'grid') {
      ...
      stylersPrimal.push(contentGridSelector + contentGridStyle); // if (params.viewType == 'grid') {
    ...
    if (params.showHeatmap) {
      for (var index in params.areas) {
        ...
        if (!heatmapMax) heatmapMax = area.number;else if (area.number > heatmapMax) heatmapMax = area.number;
        if (!heatmapMin) heatmapMin = area.number;else if (area.number < heatmapMin) heatmapMin = area.number;
      }
    } // Infobox 
    ...
    if (params.showInfobox) {
      ...
      if (infoboxDiv.length > 0) {
        infoboxDiv.addClass(params.infoboxClass).appendTo(contentDiv);
      } else {
        infoboxDiv = $('<div>').addClass(params.infoboxClass).html(params.infoboxContent).appendTo(contentDiv);
      } // Heatmap
      ...
      if (params.showHeatmap && params.showHeatlabel) {
        ...
        stylers.push(heatmapSelector + heatmapStyle); // HeatmalLabel Li
        ...
        stylers.push(heatmapLabelLiSelector + heatmapLabelLiStyle); // HeatmalLabel Li:hover
        ...
        stylers.push(heatmapLabelLiHoverSelector + heatmapLabelLiHoverStyle); // HeatmalLabel Li First Child
        ...
        stylers.push(heatmapLabelLiFirstChildSelector + heatmapLabelLiFirstChildStyle); // HeatmalLabel Li Last Child
        ...
        stylers.push(heatmapLabelLiLastChildCssSelector + heatmapLabelLiLastChildCssStyle); // HeatmalLabel Li Span
        ...
        stylers.push(heatmapLabelSpanCssSelector + heatmapLabelSpanCssStyle); // HeatmalLabel Li Span:hover
        ...
        for (var index = 0, l = params.heatmapColors.length; index < l; index++) {
          ...
          $('<li>').attr('jmap-uniq', index).append(span).appendTo(heatmapUl);
        }
    ...
    stylers.push(prefectureSelector + prefectureStyle); // Prefecture Event
    ...
    $.each(conf.prefectures, function (_index, pref) {
        ...
        if (params.heatmapConditions.length > 0 && option.number) {
          $.each(params.heatmapConditions, function (index, _cond) {
            ...
            if (eval(option.number + " " + _cond + "? true: false")) {
              ...
              if (!params.heatmapFontColors[params.heatmapColors.length - 1 - index]) option.fontColor = $(this).jmapBlackOrWhite(option.color);else option.fontColor = params.heatmapFontColors[params.heatmapColors.length - 1 - index];
              if (option.hoverColor) delete option.hoverColor;
              return false;
            }
          });
        } else if (option.number) {
          // if Number are defined in item option
          ...
          if (!params.heatmapFontColors[index]) option.fontColor = $(this).jmapBlackOrWhite(option.color);else option.fontColor = params.heatmapFontColors[index];
          if (option.hoverColor) delete option.hoverColor;
        }
      ...
      if (params.showRoundedPrefecture) prefectureIndivisualCss = $.extend(prefectureIndivisualCss, {
        'border-radius': pref.radius.join(' ').replace(/1/g, params.prefectureRadius)
      });
      if (option.fontColor) prefectureIndivisualCss['color'] = option.fontColor;
      ...
      var prefDiv = $('<div>').data('data', pref).addClass(params.prefectureClass).attr('jmap-uniq', pref.code); // Prefecture Inner
